...
function loadProject(simuid, editorUi){
	...
	}else 
	{
		...
		{			
			for(var x=0; x<objProject.length; x++)
			{
				if(simuId == objProject[x].id)
				{
					simuId = objProject[x].id;
					simuName = objProject[x].name;		
				}
			}			
		}

		//update localstorage
        var oproject={'curProId':simuId, 'newProId':''}; 
        localStorage.setItem('thpcloud.openproject', JSON.stringify(oproject));	
