...
Toolbar.prototype.init = function()
{
	...
	//run  and edit 
	var viewmodel = this.addItems(['runview', 'editview', '-']);
	//<i class="fa fa-remove tab-close" onclick="tabclose();"></i>
	viewmodel[0].setAttribute('title', mxResources.get('runview'),'','XXXXX');
	viewmodel[0].firstChild.style['background'] = 'url("static/icons/runview_icon.svg") no-repeat top left';
	viewmodel[0].firstChild.style['background-size'] = '90%';
	
	viewmodel[1].setAttribute('title', mxResources.get('editview'),'','YYYY');
	viewmodel[1].firstChild.style['background'] = 'url("static/icons/editview_icon.svg") no-repeat top left';
	viewmodel[1].firstChild.style['background-size'] = '90%';	
	
	//wizard	
	//var testitem1 = this.addItems(['wizard','-']);
	//testitem1[0].setAttribute('title', mxResources.get('wizard'),'','向导');
	// testitem1[0].firstChild.className='icon-megaphone';
	//testitem1[0].firstChild.style['background'] = 'url("static/icons/wizard_icon.svg") no-repeat top left';
	//testitem1[0].firstChild.style['background-size'] = '100%';
	
	
	//share
	var shareBtn=this.addItems(['sharebtn','-']);
	shareBtn[0].firstChild.className='icon-share';
	shareBtn[0].firstChild.style['background-size'] = '100%';
	shareBtn[0].firstChild.style['font-weight'] = '700';
	shareBtn[0].firstChild.style['font-size'] = '16px';
	shareBtn[0].firstChild.style['padding-top'] = '3px';
	// var shareIco=document.createElement('i');
	// shareIco
	
	//save option	
	if(!this.editorUi.isModule){
		var testitem = this.addItems(['qsave','qsave_history','-']);
		testitem[0].setAttribute('title',mxResources.get('qsave','','快速保存算例'));
		testitem[0].firstChild.style['background'] = 'url("static/icons/content-save.svg") no-repeat top left';
		testitem[0].firstChild.style['background-size'] = '100%';
		
		testitem[1].firstChild.style['background'] = 'url("static/icons/content-save-settings.svg") no-repeat top left';
		testitem[1].firstChild.style['background-size'] = '100%';
		testitem[1].setAttribute('title',mxResources.get('qsave_history','','快速保存快照'));
	}
	...
	*/
	...
	var multistateitem = this.addItems(['multistate','lineDynamicStyle','printArea']);
	multistateitem[0].setAttribute('title', mxResources.get('multistate','','多态配置'));
	multistateitem[0].firstChild.style['background'] = 'url("static/icons/multistate.png") no-repeat top left';
	multistateitem[0].firstChild.style['background-size'] = '100%';
	multistateitem[1].setAttribute('title', mxResources.get('lineDynamicStyle','','线路动态样式'));
	multistateitem[1].firstChild.style['background'] = 'url("static/images/icons48/table.png") no-repeat top left';
	multistateitem[1].firstChild.style['background-size'] = '100%';
    multistateitem[2].setAttribute('title', mxResources.get('printArea','','打印'));
    multistateitem[2].firstChild.style['background'] = 'url("static/images/printer.png") no-repeat top left';
    multistateitem[2].firstChild.style['background-size'] = '100%';
...
/*	var search_input = this.addItems(['selectNone']);
	search_input[0].innerHTML = '<div style="padding: 2px 30px 0px 0px;float: right;width: 218px;white-space: nowrap;text-overflow: clip;padding-bottom: 8px;cursor: default;">\
...
/*	mxUtils.bind(elts1[0],function(){
		console.log('dosomething');
	});
*/
	// var ds = mxUtils.makeDraggable(elts1[0], _graph, addIndexTxt, dragElt, null, null, _graph.autoscroll, true);
	// var ds2 = mxUtils.makeDraggable(elts1[1], _graph, Dividingline, dragElt, null, null, _graph.autoscroll, true);
	// var ds3 = mxUtils.makeDraggable(elts1[2], _graph, addSimuResult, dragElt, null, null, _graph.autoscroll, true);
	// var ds4 = mxUtils.makeDraggable(elts1[3], _graph, addSimuCalResult, dragElt, null, null, _graph.autoscroll, true);
// 	ds.isGuidesEnabled = function()
// 				{
// 					return _graph.graphHandler.guidesEnabled;
// 				};

// 				// Restores original drag icon while outside of graph
// 	ds.createDragElement = mxDragSource.prototype.createDragElement;
// 	ds2.isGuidesEnabled = function()
// 				{
// 					return _graph.graphHandler.guidesEnabled;
// 				};
...
Toolbar.prototype.addLineSelect = function()
{
	var testitem = this.addItems(['lineA','lineB','lineC','-']);
	testitem[0].setAttribute('title','Line1');
	testitem[1].setAttribute('title','line2');
	testitem[2].setAttribute('title','line3');
	testitem[0].firstChild.className = 'geSprite geSprite-linkedge';
	testitem[1].firstChild.className = 'geSprite geSprite-arrow';
	testitem[2].firstChild.className = 'geSprite geSprite-simplearrow';
	this.LineSelectElements = testitem;
}

Toolbar.prototype.removeLineSelect = function(){
	var elements = this.LineSelectElements;
	if(elements!=null){
...
/*Toolbar.prototype.addSearch = function(c)
{
	...
	//inner.style.padding = '2px 30px 0px 0px';
	inner.style.float = 'right';
	
	var input = document.createElement('input');
	input.setAttribute('placeholder', mxResources.get('searchShapes'));
	//input.setAttribute('placeholder', 'Search Component');
	input.setAttribute('type', 'text');
	input.setAttribute('id','searchele');
	...
	cross.setAttribute('src',searchimage);
	...
	function getAllEle(){
	   ...
	   // var vts = _graph.getChildVertices();
	    ...
	    _rpssEle.sort();
	    var _rpssEle_e = [];
	    for (var x =0 ;x<_rpssEle.length;x++){
					var content = {};
	    			content.label = _rpssEle[x];
	    			content.value = _rpssEle[x];
	    			_rpssEle_e.push(content);
	    }
	    allele = allele.concat(_rpssEle_e);
	    var pin_sum = []
	    for (var x =0;x<_rpssEle.length;x++){
	    	var pin = _pssEle[_rpssEle[x]].pin;
	    	for (var y in pin){
	    		if (pin[y].label!=''&&pin[y].label!=undefined){
	    			var content = {};
	    			content.label = _rpssEle[x]+' port:'+pin[y].label;
	    			content.value = _rpssEle[x];
	    			//[_rpssEle[x]+'port:'+pin[y].label,_rpssEle[x]];
	    			pin_sum.push(content);
	    		}
	    	}
	    }
	    allele = allele.concat(pin_sum);
	    return allele;
	}
	
	var awesomplete = new Awesomplete(input);
	awesomplete.list = getAllEle();
	input.addEventListener('awesomplete-selectcomplete',function(){

	  	//alert(this.value);
	  	var cell = searchcellbyname(this.value);
	  	focusTargetCell(cell);
	});
