...
//Added by wugh
mxCell.prototype.isOpen = 1;
mxConstraintHandler.prototype.pointImage = new mxImage(mxClient.imageBasePath + '/point.gif', 10, 10);
...
Graph = function(container, model, renderHint, stylesheet, themes)
{
	...
	//此处新增，解决mxgraph版本升级带来的bug
	this.cellRenderer.defaultShapes = mxCellRenderer.defaultShapes;
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		// Prohibition against group deep recursion.--zhanglw 2018.07.26
		...
		this.selectRegion = function(rect, evt)
		{
            ...
            cells.forEach(function (cell) {
                ...
                while (model.isVertex(parent) && (!graph.isValidRoot(parent) || parent.style.indexOf('group')>=0)) {
                    cell = parent;
			...
			this.selectCellsForEvent(cellList, evt);
...
// true ---> false , modified by wugh at 2018.05.21
...
{
	// Prevent cross-border calculation --zhanglw 2018.8.15
...
{
	return cell.value != null && typeof(cell.value) == 'object' && cell.value.getAttribute &&
...
{
	//dx加载echarts
	 if (cell.value&&(cell.value.constructor===GModel.GChart||cell.value instanceof GModel.GChart))
    {
        var node = document.createElement('div');
        node.setAttribute('style', 'width:'+cell.geometry.width+"px;height:"+cell.geometry.height+"px;padding:10px;");
    ...
    if (cell.value && (cell.value.constructor === GModel.GLinear || cell.value instanceof GModel.GLinear)) {
        var node = document.createElement('div');
        node.name = 'gobj';
    ...
    if (cell.value && (cell.value.constructor === GModel.GMap || cell.value instanceof GModel.GMap)) {
        var node = document.createElement('div');
        node.name = 'gobj';
        if(this.isEnabled()){
 ...
 * 添加直接选择父层函数
 * @Author   Wugh
 * @DateTime 2017-11-02T17:51:32+0800
 * @return   {[type]} [description]
 */
Graph.prototype.getSelectParents = function() {
        var cells = this.getSelectionModel().cells.slice();

        function parentCell(obj) {
            if ("0" != obj.parent.parent.id)
                return parentCell(obj.parent);
            else {
                return obj;
            }
        }
        ...
        for (var i = 0; i < cells.length; i++) {
            if (cells[i].parent) {
                var tempParent = parentCell(cells[i])
...
mxStencilRegistry.getStencil = function(name)
{
	...
	//旧版本为mxCellRenderer.prototype.defaultShapes
	//新版本为mxCellRenderer.defaultShapes
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
		...
		mxGraphHandler.prototype.moveCells = function(cells, dx, dy, clone, target, evt)
		{
            ...
            bounds.maxX+=dx;
            bounds.maxY+=dy;
            if(bounds.minX<graph.pageFormat.x || bounds.minY<graph.pageFormat.y ||
                bounds.maxX>graph.pageFormat.x+graph.pageFormat.width || bounds.maxY>graph.pageFormat.y+graph.pageFormat.height){
                mxEvent.consume(evt);
                return;
			}
			// END
