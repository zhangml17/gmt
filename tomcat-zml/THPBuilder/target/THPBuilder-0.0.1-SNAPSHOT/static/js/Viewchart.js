...
Viewchart={
    ...
    createChart:function (points,viewDiv) {
        ...
        for(var index  in points){
            if(recordids.indexOf(points[index].id) == -1){
                ids.push(parseInt(points[index].id));
            }
        ...
        this.getDatabyServer(tbname,ids,pretime,time);
    ...
    selectData:function () {
        ...
        // var enddate=Date.parse(time);
        var tbname="hdr_analog";
        Viewchart.getDatabyServer(tbname,recordids,pretime,time);
    },
    /*获取历史数据并加载到chart图表中*/
            ...
            if(req && req.request && req.request.responseText ){
                ...
                if(da && da.replyCode.code=="0" && da.result ) {
                    ...
                    if (data) {
                        ...
                        charts.initDataToChart(data, Viewchart.chartId);
                    }
                }
