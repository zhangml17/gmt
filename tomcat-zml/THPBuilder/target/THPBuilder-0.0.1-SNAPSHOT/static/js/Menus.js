 ...
 * Sets the default font size.
 */
Menus.prototype.defaultMenuItems = ['file', 'view', 'edit', 'draw', 'arrange', 'tool', 'store', 'help'];
...
Menus.prototype.init = function()
{
	...
	{
		//this.addMenuItems(menu, ['save', 'saveAs', '-', 'import', 'export', '-', 'pageSetup', 'print'], parent);
		//this.addMenuItems(menu, ['open', 'new',  '-', 'save', 'saveAs', '-', 'import', 'export', '-', 'setup', 'preview', 'print','-', 'exit'], parent);
		this.addMenuItems(menu, ['open', 'new',  '-', 'save', '-', 'import', 'export', '-', 'setup', 'preview', 'print','-', 'exit'], parent);
	})));
	...
	this.put('view', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		...
		this.addMenuItems(menu, ((this.editorUi.sidebar != null) ? ['sidebarPanel'] : []).
			concat([((this.editorUi.format != null) ? ['formatPanel'] : []), 'outline', 'layers', '-', 'pageView', 'pageScale', '-', 'scrollbars', 'tooltips', '-',
			        'grid', 'guides', '-', 'display', '-', 'connectionArrows', 'connectionPoints', '-',
	...
	this.put('edit', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['undo', 'redo', '-', 'cut', 'copy', 'paste', 'delete', 'duplicate', '-',
		                         'editTooltip', 'editStyle', '-',  'editLink', 'openLink', '-',
		                         'selectVertices', 'selectEdges', 'selectAll', 'selectNone', '-', 'lockUnlock']);
		//'editData',  '-',
	})));
	
	//4-绘图
	this.put('draw', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		//this.addMenuItems(menu, ['copyConnect', 'collapseExpand', '-', 'editDiagram']);
		this.addSubmenu('font', menu, parent);
		this.addSubmenu('color', menu, parent);
		this.addSubmenu('format', menu, parent);
		
		this.addMenuItems(menu, ['-', 'setDefaultStyle', 'clearDefaultStyle']);
		this.addMenuItems(menu, ['-', 'multistate', 'elecstate']);
		
	})));
	
	this.put('font', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['fontColor', 'bold', 'italic', 'underline'], parent);
	})));
	this.put('color', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['strokeColor', 'fillColor', 'gradientColor', 'backgroundColor' , 'borderColor' ], parent);
	})));
	this.put('format', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['vertical', 'shadow', 'solid'], parent);
	})));
	...
	//6-工具
	this.put('tool', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['runview', 'editview', '-', 'datasource', 'timerange', 'tagsearch', '-',  'editdata', 'editscript', '-'], parent);
		this.addSubmenu('language', menu, parent);
		this.addSubmenu('skin', menu, parent);		                         
		this.addMenuItems(menu, ['-', 'preference'], parent);
	})));
	
	this.put('language', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['languageZH', 'languageEN'], parent);
	})));
	
	this.put('skin', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['skingray', 'skinblue'], parent);
	})));
	...
	//7-图元
	this.put('store', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['metasearch', '-', 'importpic', 'browsepic', '-', 'newmeta', 'editmeta', '-']);
		this.addSubmenu('basemeta', menu, parent);
		this.addSubmenu('dynameta', menu, parent);	
		this.addSubmenu('elecmeta', menu, parent);
		this.addSubmenu('equipmeta', menu, parent);	
		
	})));
	this.put('basemeta', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['metaa', 'metab'], parent);
	})));
	this.put('dynameta', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['metaa', 'metab'], parent);
	})));
	this.put('elecmeta', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['metaa', 'metab'], parent);
	})));
	this.put('equipmeta', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		this.addMenuItems(menu, ['metaa', 'metab'], parent);
	})));
	...
	{
		this.addMenuItems(menu, ['help', 'online','-', 'about']);
	})));
	...
	this.put('viewPanels', new Menu(mxUtils.bind(this, function(menu, parent)
	{
		...
		{
			this.addMenuItems(menu, ['sidebarPanel'], parent);
		}
