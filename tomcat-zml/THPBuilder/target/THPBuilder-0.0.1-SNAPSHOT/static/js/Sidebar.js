...
Sidebar.prototype.addCustomPalette = function (metaId, metaContent, metaName) {
    ...
    // 获取面板台头
    var title = contentDiv.parentNode.previousSibling; //contentDiv  这个是包含添加和编辑图片按钮的div元素 这里的title就是a标签
...
Sidebar.prototype.addDynamicPalette = function (metaId, metaContent, metaName) {
    ...
    var _this=this;
    var list=[];
    var title=['gobjDataTag','gobjImage','gobjButton','gobjChartTrend','gobjChartBar','gobjChartPie','gobjChartDisperse','gobjChartGauge','gobjLinear','gobjMap','gobjDataTable'];
    var index=0;
    cells.forEach(function(cell){
    	list.push(         
    			_this.addEntry('', mxUtils.bind(_this, function () {
...
Sidebar.prototype.addUmlPalette = function(expand)
{
	...
	var fns = [
		...
		this.addEntry('uml activity condition', function()
		{
            ...
            // console.log(sb.createVertexTemplateFromCells([cell, edge1, edge2], 180, 100, 'Condition'),'这是啥');
...
Sidebar.createTitle;
/**
 * Creates a thumbnail for the given cells.   为给定图元创建一个缩略图
 */
Sidebar.prototype.createThumb = function(cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight)
{
    ...
    //illustrate to model. --zhanglw 2018.8.29
    var gobj=cells[0].value;
    if(gobj instanceof GModel.GChart){
        node = document.createElement('div');
        node.style.backgroundImage='url(./static/images/assembly/'+gobj.gobjType+'.png)';
        node.style.backgroundPosition ='center center';
    }else if(gobj instanceof GModel.GDataTag){
        node = document.createElement('div');
        node.style.backgroundImage='url(./static/images/assembly/dataTag.png)';
        node.style.backgroundPosition ='center center';
    }else if(gobj instanceof GModel.GLinear){
        node = document.createElement('div');
        node.style.backgroundImage='url(./static/images/assembly/linear.png)';
        node.style.backgroundPosition ='center center';
    }else if(gobj instanceof GModel.GMap){
        node = document.createElement('div');
        node.style.backgroundImage='url(./static/images/assembly/map.png)';
        node.style.backgroundPosition ='center center';
    }else if(gobj instanceof GModel.GButton){
        node = document.createElement('div');
        node.style.backgroundImage='url(./static/images/assembly/button.png)';
        node.style.backgroundPosition ='center center';
    }else if(gobj instanceof GModel.GImage){
        node = document.createElement('div');
        node.style.backgroundImage='url('+gobj.url+')';
        node.style.backgroundPosition ='center center';
    }
    //END
...
/**
 * Creates and returns a new palette item for the given image.  创建并返回给定图像的新调色板项
 */
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	...
	{
		var ds = this.createDragSource(tab, this.createDropHandler(cells, true, allowCellsInserted,
			bounds), this.createDragPreview(width, height), cells, bounds);
		this.addClickHandler(tab, ds, cells);
	...
	{
		var ds = this.createDragSource(tab, this.createDropHandler(cells, false, allowCellsInserted,
			bounds), this.createDragPreview(width, height), cells, bounds);
		this.addClickHandler(tab, ds, cells);
    }
	// delete tooltip --zhanglw 2018.8.16
	// Shows a tooltip with the rendered cell
	if (!mxClient.IS_IOS)
	{
		mxEvent.addGestureListeners(tab, null, mxUtils.bind(this, function(evt)
		{
...
/**
 * Creates a drop handler for inserting the given cells.  创建用于插入给定单元格的下拉处理程序
 */
...
Sidebar.prototype.itemClicked = function(cells, ds, evt, elt)
{
	...
	else
	{
        ...
        /*ds.drop(graph, evt, null, pt.x, pt.y);这是之前图元在页面出现的x、y轴坐标*/
        ds.drop(graph, evt, null,0,0)
 ...
 * will be filtered out later.  为给定模板创建标签数组。允许复制和稍后将被过滤掉。
 */
