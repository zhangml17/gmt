...
CreateTable={
    ...
    create_Table:function (tableName,viewDiv) {
        ...
        mxUtils.get(settings.snapServiceURL+"/getModelDef", function(req){
...
//                CreateTable.get_tablemodel(tableName,viewDiv);
    ...
    get_tablemodel: function (tablename,pointID,viewDiv) {
        ...
        var contentStr = JSON.stringify(param);
        mxUtils.post(settings.snapServiceURL+"/getRecordByIndexfilter", contentStr, function(req){
        // mxUtils.get("http://39.107.241.142:3501/webgis/conditionget_tablemodel", contentStr, function(req){
            if(req.request && req.request.responseText ){
                ...
                if(data && data.replyCode.code=="0" && data.result && data.result.data){
                    var html= CreateTable.CreateTableData(tablename,data.result.data);
    ...
    CreateTableTitle: function (TablesColumn,TableName) {
        for (var i = 0; i < TablesColumn.length; i++) {
            if (TableName == TablesColumn[i].tableEname) {
                ...
                //ul实现
                var html = '<div class="ul-box"  > ' +
                        '<ul class="ul-table" id="'+TableName+'">';
                html += '<li class="litop"><span class="span-left">名称</span><span class="span-right" style="text-align: center;">值</span></li>';
//                for (var j = 0; j < TablesColumn[i].Columns.length; j++) {
//                    html += '<li class="num"><span>' + TablesColumn[i].Columns[j] + '</span><span name="value"></span></li>';
//                }
                html += '</ul></div>';
                this.TableID = TablesColumn[i].tableEname;
                this.TablesColumns=TablesColumn[i].Columns;
                return html;
                break;
    ...
    /*获取表中数据*/
    CreateTableData:function (tablename,data) {
        var html = "";
        if(data){
             //html = '<div class="table-box" style="overflow-y:auto;width:100%;margin-top: 20px;"  ' +
             // html = '<ul id="'+tablename+'">';
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < content.length; j++) {
                    ...
                    if(this.TablesColumns[j]){
                    	  // html += '<li class="num"><span>' + this.TablesColumns[j] + '</span><span>'+value+'</span></li>';
                       html += '<li class="num"><span class="span-left" >' + this.TablesColumns[j] + '：</span><span class="span-right" >'+value+'</span></li>';
                    }
    ...
    /*获取表中数据*/
    CreateTableData1:function (data) {
        var html = "";
        if(data){
            for (var i = 0; i < data.length; i++) {
                if (i / 2 == 0 && i != 0) {
