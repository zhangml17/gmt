...
EditorUi = function(editor, container, lightbox)
{
	...
	if (this.container == document.body)
	{
		...
		// Added by wugh
		this.labelContainer.onselectstart=textEditing;
		this.labelContainer.onmousedown=textEditing;
	...
	// Adds tooltip when mouse is over scrollbars to show space-drag panning
	// option
	...
	// Ctrl/Cmd+Enter applies editing value except in Safari where Ctrl+Enter
	// creates
	...
	// Note: Everything that is not in styles is ignored (styles is augmented
	// below)
	...
	// Keys that should be ignored if the cell has a value (known: new default
	// for all cells is html=1 so
    // for the html key this effecticely only works for edges inserted via the
	// connection handler)
	...
	// Implements a global current style for edges and vertices that is applied
	// to new cells
		...
		try
		{
			...
			else
			{
				for (var i = 0; i < cells.length; i++)
				{
					...
					// Removes styles defined in the cell style from the styles
					// to be applied
					...
					for (var j = 0; j < tokens.length; j++)
					{
						var tmp = tokens[j];
				 		...
				 		if (pos >= 0)
				 		{
				 			...
				 			// Handles special cases where one defined style
					...
					for (var j = 0; j < appliedStyles.length; j++)
					{
						...
						{
							// Special case: Connect styles are not applied here
							// but in the connection handler
	...
	// Updates the editor UI after the window has been resized or the
	// orientation changes
	// Timeout is workaround for old IE versions which have a delay for DOM
	// client sizes.
	// Should not use delay > 0 to avoid handle multiple repaints during window
	// resize
...
/**
 * Added by wugh Specifies the height of the label. Default is 28.
 */
EditorUi.prototype.labelHeight = 28;
...
/**
 * Specifies the width of the sidebar panel. Default is 208.
 * added by wugh
 */
EditorUi.prototype.sidebarWidth = (screen.width <= 640) ? 118 : 208;
...
/**
 * Returns true if the given event should start editing. This implementation
 * returns true.
 */
...
EditorUi.prototype.initCanvas = function()
{
	...
	{
		return new mxPoint(Math.max(0, Math.round(15 / graph.view.scale)),
				Math.max(0, Math.round(15 / graph.view.scale)));
		
		//modified by wugh, at 20180523
		//no padd for ruler: ruler height15 and border 1 px
		// return new mxPoint(15,15);
	...
	if (this.editor.chromeless)
	{
		...
		// Installs handling of hightligh and handling links to relative links
		// and anchors
	...
	{
		/**
		 * Guesses autoTranslate to avoid another repaint (see below). Works if
		 * only the scale of the graph changes or if pages are visible and the
		 * visible pages do not change.
		 */
		...
		graph.view.validate = function()
		{
			if (this.graph.container != null && mxUtils.hasScrollbars(this.graph.container))
			{
				...
				// Updating scrollbars here causes flickering in quirks and is
				// not needed
				// if zoom method is always used to set the current scale on the
				// graph.
		...
		graph.sizeDidChange = function()
		{
			if (this.container != null && mxUtils.hasScrollbars(this.container))
			{
				...
				if (!this.autoTranslate && (this.view.translate.x != dx || this.view.translate.y != dy))
				{
					...
					// IS NO WAY AROUND THIS SINCE THE
					// BOUNDS ARE KNOWN AFTER THE VALIDATION AND SETTING THE
					// TRANSLATE TRIGGERS A REVALIDATION.
	...
	graph.lazyZoom = function(zoomIn)
	{
		...
		// set ruler scale --zhanglw 2018.8.17
        window.setTimeout(function () {
            editorUi.myRuler.api.setScale(1/graph.view.scale);
        },40);
        // END
	};
	...
	{
		// Ctrl+wheel (or pinch on touchpad) is a native browser zoom event is
		// OS X
...
/**
 * wugh, added 2018.5.15
 * 
 */
EditorUi.prototype.toggleSidebarPanel = function(forceHide)
{
	this.sidebarWidth = (forceHide || this.sidebarWidth > 0) ? 0 : (screen.width <= 640) ? 118 : 208;
	this.sidebarContainer.style.display = (forceHide || this.sidebarWidth > 0) ? '' : 'none';
	this.refresh();
...
/**
 * Installs dialog if browser window is closed without saving This must be
 * disabled during save and image export.
 */
...
EditorUi.prototype.resetScrollbars = function()
{
	...
	else if (!this.editor.chromeless)
	{
		...
		{
			// This code is not actively used since the default for scrollbars
			// is always true
			if (graph.pageVisible)
			{
...
EditorUi.prototype.updateActionStates = function()
{
	...
	var actions = ['cut', 'copy', 'bold', 'italic', 'underline', 'delete', 'duplicate',
	               ...
	               'editview', 'toFront', 'toBack', 'lockUnlock', 'solid', 'dashed',
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	var diagramHeight1 = Math.max(0, h - this.footerHeight - this.labelHeight - this.menubarHeight - this.toolbarHeight);
	
	// Added by wugh
	// 2017.11.09
	this.labelContainer.style.left= effHsplitPosition+this.splitSize + 'px';	
	var diagramWidth1 = Math.max(0, w - effHsplitPosition - fw);
	
	this.labelContainer.style.width=diagramWidth1 + 'px';
	this.labelContainer.style.height=this.labelHeight + 'px';	
	this.labelContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
	this.labelContainer.style.top = this.sidebarContainer.style.top;

	
	this.diagramContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
	this.diagramContainer.style.top = (this.footerHeight + this.labelHeight + this.menubarHeight + this.toolbarHeight) + 'px';
	this.diagramContainer.style.height = diagramHeight1 + 'px';
	...
	if (quirks)
	{
		...
		var diagramHeight = Math.max(0, h - this.footerHeight - this.labelHeight - this.menubarHeight - this.toolbarHeight);
	...
	else
	{
		...
		// this.sidebarContainer.style.bottom = (this.footerHeight +
		// sidebarFooterHeight + off) + 'px';
...
{	
	// Added by wugh
	// logo div
	this.thplogo = this.createDiv('geThpLogo');
	this.thplogo.setAttribute('title', '国科恒通');
	...
	// Added by wugh
	this.labelContainer=this.createDiv('geFooterContainer');
	...
	this.thplogo.style.top = '0px';
	this.thplogo.style.left = '0px';
	this.thplogo.style.width = '208px';
	this.thplogo.style.height = '64px';
	
	
	this.menubarContainer.style.top = '0px';
	this.menubarContainer.style.left = '208px';
	this.menubarContainer.style.right = '0px';
	this.toolbarContainer.style.left = '208px';
...
{
	// Added by wugh
	var simudiv = document.createElement('div');
	simudiv.id = 'simuinfocontainer';
	simudiv.style.height = '30px';
	simudiv.style['padding'] = '0px 0px 0px 400px';
	
	// var bannerContainer = '<div class="logocolor" style="/*background-color:
	// #4285f4;*/height: 60px;position: absolute;width: 209px;z-index: 1;">\
    // <a href="/main/" title="Cloudpss Home" data-tooltip="Cloudpss home"
	// aria-label="Cloudpss home">\
    // <img src="/static/icons/banner_logo.png" height="100%" width="100%" />\
    // </a>\
    // </div>';
    // <div id="docs-branding-logo" style="background:
	// url(/static/icons/banner_logo.png) center no-repeat;width:210px"></div>
    var textContainer = '<div id="SimuInfotext" style="padding:0px 0px 0px 226px">'+mxResources.get('NewProject')+'</div>';
    // simudiv.innerHTML = bannerContainer+textContainer;
    simudiv.innerHTML = textContainer;

   // var esdiv = document.createElement('div');
   // esdiv.style.width = '209px';
   // esdiv.style.height= '35px';
   // esdiv.style.float = 'left';
   // esdiv.style.color = 'white';
   // esdiv.innerHTML = '<center style="margin: 0px 526px 0px 0px;">Powered By
   // THPower Co.LTD.</center>';
   // esdiv.setAttribute('class','logocolor');
	...
	{
		// Add by wugh
		// add information : project name and company name
		// this.menubarContainer.appendChild(simudiv);
		// this.menubarContainer.appendChild(esdiv);
		
		// this.menubar.container.appendChild(simudiv);
	...
	// wuguanhui
	// Creates logo
	this.container.appendChild(this.thplogo);
	...
	// add by wugh
	// display label:
	this.label = (this.editor.chromeless) ? null : this.createLabel(this.labelContainer);
...
{
	return new SidebarBast(this, container);
...
/*
 * Add by wugh creates a new label for the given container
 */
EditorUi.prototype.createLabel = function(container){
...
/*
 * added by wugh Displays a login status
 */

EditorUi.prototype.loginstatus = function(argument){
	if (loginResult==null){
		$.get("/THPBuilder/login_status", {}, function(result) {
            if (result.is_login) {
            	loginResult = result;
            	this.addLoginStatus();
            }
...
EditorUi.prototype.addLoginStatus = function(c)
{
	c = (c != null) ? c : document.body;
	if(loginResult.is_login){
		...
		chContainer.style.right = '30px';
		chContainer.style.top = '5px';
		chContainer.style.height = '25px';
		chContainer.style.zIndex = '33';
		
		var nameContainer = document.createElement('div');
		nameContainer.style.textShadow = '0 1px 0 #FFF';
		nameContainer.style.color = '#525252';
		nameContainer.style.fontWeight = 'bold';
		nameContainer.style.lineHeight = '25px';
		nameContainer.style.float = 'left';
		nameContainer.style.position = 'relative';
		nameContainer.style.paddingRight = '10px';
		nameContainer.style.cursor = 'pointer';
		nameContainer.style.minWidth = '100px';
		
		var img = document.createElement('img');
		img.setAttribute('src',loginResult.head_portrait);
		...
		ico.className = 'icon-chevron-small-down';
		ico.style.marginTop = '0px';
		ico.style.float = 'left';
		// ico.style.width = '0';
		// ico.style.height = '0';
		// ico.style.borderLeft = '5px solid transparent';
		// ico.style.borderRight = '5px solid transparent';
		// ico.style.borderTop = '10px solid #999';
		
		nameContainer.appendChild(img);
		nameContainer.appendChild($('<span style="float:left">'+loginResult.username+'</span>')[0]);
		nameContainer.appendChild(ico);
		chContainer.appendChild(nameContainer);
		
		// mxUtils.write(nameContainer,loginResult.username);
		...
		ul.style.right = '0px';
		ul.style.padding = '4px 0px';
		ul.style.position = 'absolute';
		ul.style.background = '#FFF';
		ul.style.borderRadius = '4px';
		ul.style.border = '1px solid rgb(180,180,180)';
		ul.style.margin = '0px';
		...
		li2 = li.cloneNode(false);
		lidivider = li.cloneNode(false);
		lidivider.style.height = '0px';
		lidivider.style.borderTop = '#ccc solid 1px';
		lidivider.style.margin = '4px 0';
		mxUtils.write(li,'我的首页');
		mxUtils.write(li2,'退出');
		ul.appendChild(li);
		ul.appendChild(lidivider);
		ul.appendChild(li2);
		chContainer.appendChild(ul);
		mxEvent.addListener(li, 'click',  function(e){
			window.location.href = "main.html";
		});
		mxEvent.addListener(li2, 'click',  function(e){
			window.location.href = "main.html?id=0";
		});
		document.addEventListener('click', function(e){
			var parent=$(e.target).parents('#chContainer');
			var isshow = $(ul).css('display');
			if(parent.length == 0){
				if(isshow != 'none')$(ul).hide();
			}else{
				if(isshow == 'none')$(ul).show();
			}
		});
		
		c.appendChild(chContainer);
	}
}
...
/**
 * Hook for subclassers to return true if event data is a supported format. This
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	{
		// Handles undo/redo/ctrl+./,/u via action and allows ctrl+b/i only if
		// editing value is HTML (except for FF and Safari)
			...
			if (!graph.isSelectionEmpty() && graph.isEnabled())
			{
				...
				else
				{
					...
					else
					{
                        ...
                        // Beyond the boundary stop move --zhanglw 2018.8.15
                        var bounds={
                        ...
                        graph.getSelectionCells().forEach(function (cell) {
                            ...
                            if(graph.model.isVertex(cell)){
                                if(geometry.x<bounds.minX){bounds.minX=geometry.x}
                                if(geometry.y<bounds.minY){bounds.minY=geometry.y}
                                if(geometry.x+geometry.width>bounds.maxX){bounds.maxX=geometry.x+geometry.width}
                                if(geometry.y+geometry.height>bounds.maxY){bounds.maxY=geometry.y+geometry.height}
                            }
                            if(graph.model.isEdge(cell)){
                                if(geometry.sourcePoint.x<bounds.minX){bounds.minX=geometry.sourcePoint.x}
                                if(geometry.sourcePoint.y<bounds.minY){bounds.minY=geometry.sourcePoint.y}
                                if(geometry.sourcePoint.x>bounds.maxX){bounds.maxX=geometry.sourcePoint.x}
                                if(geometry.sourcePoint.y>bounds.maxY){bounds.maxY=geometry.sourcePoint.y}
                                if(geometry.targetPoint.x<bounds.minX){bounds.minX=geometry.targetPoint.x}
                                if(geometry.targetPoint.y<bounds.minY){bounds.minY=geometry.targetPoint.y}
                                if(geometry.targetPoint.x>bounds.maxX){bounds.maxX=geometry.targetPoint.x}
                                if(geometry.targetPoint.y>bounds.maxY){bounds.maxY=geometry.targetPoint.y}
                            }
                        });
                        bounds.minX+=dx;
                        bounds.minY+=dy;
                        bounds.maxX+=dx;
                        bounds.maxY+=dy;
                        if(bounds.minX<graph.pageFormat.x || bounds.minY<graph.pageFormat.y || bounds.maxX>graph.pageFormat.x+graph.pageFormat.width || bounds.maxY>graph.pageFormat.y+graph.pageFormat.height){
                            return;
                        }
                        // END
	...
	mxKeyHandler.prototype.getFunction = function(evt)
	{
		if (graph.isEnabled())
		{
			// TODO: Add alt modified state in core API, here are some specific
			// cases
			...
			else if (directions[evt.keyCode] != null && !graph.isSelectionEmpty())
			{
				...
				{
					// Avoids consuming event if no vertex is selected by
...
EditorUi.prototype.destroy = function()
{
	...
	var c = [this.menubarContainer, this.toolbarContainer, this.sidebarContainer,
	         ...
	         this.chromelessToolbar, this.hsplit, this.thplogo,this.sidebarFooterContainer,
	         this.layersDialog];
...
EditorUi.prototype.loadSimu=function(){
	this.label.init();
}
EditorUi.prototype.loadReference = function(){
	var config = this.getLocalStorageConfig();
	if('pageView' in config && config.pageView == false){
		this.actions.actions.pageView.funct();
	}
	if('formatPanel' in config && config.formatPanel == 0){
		this.toggleFormatPanel();
	}
};
EditorUi.prototype.getLocalStorageConfig = function(){
	// body...
...
EditorUi.prototype.setLocalStorageConfig = function(config_key,config_value){
	var config = this.getLocalStorageConfig();
	config[config_key] = config_value;
	localStorage.setItem('config', JSON.stringify(config));
};
EditorUi.prototype.saveWizardPage = function(selectInfo){
	var this_ = this;
	var div = document.createElement('div');
	var form = $('<div class="container">\
				<div class="selection" style="width:100%;height:80px;border-bottom: 1px solid #d8d7d7;"><div style="width:45%;float:left;padding:5px;text-align:center"><p style="margin: 7px 0px 7px 0px; text-align: center;">请选择元件：</p><select id="cmpid" class="param-inpt" style="margin:0 auto;height:28px" ></select></div><div  style="width:45%;float:left;padding:5px;text-align:center"><img height="60px" width="60px" id="cmpimg" src=""></div></div>\
				<div style="padding:20px;border-bottom: 1px solid #d8d7d7;"><label style="width:45%;text-align:center">请输入名称</label><input class="param-inpt" id="compName" value="" type="text" name="compName" style="max-width:none;width:180px" /></div>\
				<div style="padding:20px;text-align:center"><button id="saveWizardBtn" class="geBtn gePrimaryBtn">确定</button><button id="cancelBtn" class="geBtn">取消</button></div>\
				</div>');
	$(div).append(form);

	//
	form.find('select#cmpid').change(function(){
		var icon = $(this).find("option:selected").data('icon');
		form.find('img#cmpimg').attr('src',icon);
	});
	function getComponentList(){
		$.get('/editor/componentList/?page=1',function(data){
			for (var x = 0; x < data.cmp.length; x++){
				var option = $('<option value="'+data.cmp[x].id+'">'+data.cmp[x].typename+'</option>');
				option.data('icon',data.cmp[x].icon);
				form.find('select#cmpid').append(option);
			}
			form.find('select#cmpid').val(data.cmp[0].id);
			form.find('img#cmpimg').attr('src',data.cmp[0].icon);
		});
	}
	getComponentList();

	//
	if(selectInfo.type == 'Comp' || selectInfo.type =='module'){
		form.find('input#compName').val(selectInfo.name);
	}else{
		form.find('input#compName').attr('placeholder','请填入拓扑名称');
	}


	// getimg
	 var getImg=mxUtils.bind(this,function(){
		imgUrl = { 'simu': {}, 'result': [] };
		imgUrl['simu']['xxx']=[]
		this_.exportImage(null, null, false, null, null, null, null, null, null, 'xxx');
		console.log(imgUrl['simu']['xxx'][0]);
	});
	getImg();

	form.find('button#cancelBtn').click(function(){
		savePanel.hide();
	});
	form.find('button#saveWizardBtn').click(function(){
		if (form.find('input#compName').val()==''){
			alert('名称不能为空');
			return;
		}
		var data = {};
		data.csrfmiddlewaretoken = $('input[name="csrfmiddlewaretoken"]').val();
		data.pid = form.find('select#cmpid').val();
		data.name = form.find('input#compName').val();
		data.type = selectInfo.type;
		if (selectInfo.type == 'Comp') {
			data.pic = form.find('select#cmpid').find('option[value='+selectInfo.value+']').data('icon');
			var sample = {'id':selectInfo.value};
			data.content = JSON.stringify(sample);
		}else if (selectInfo.type =='module'){
			data.pic = imgUrl['simu']['xxx'][0];
			var sample = {'id':selectInfo.value};
			data.content = JSON.stringify(sample);
		}else {
			data.pic = imgUrl['simu']['xxx'][0];
			data.content =  JSON.stringify(selectInfo.value);
		}
		console.log(data);
		$.post('/editor/saveComponentWizard/',data,function(data){
			console.log(data);
			if(data.status == 1){
				alert(data.msg);
				savePanel.hide();
	...
	var savePanel= new mxWindow('', div,  _WIN.width * 0.5 - 150, _WIN.height * 0.5 - 200, 350, null, false, true);
	savePanel.setTitle(mxResources.get('setting'));
	savePanel.setClosable(true);
	savePanel.show();
};
...
EditorUi.prototype.Intro = function(argument){
	// body...
	 var intro = introJs();
          intro.setOptions({
            ...
            steps: [
              {
              	element:".sidebartabs_",
                intro: "切换标签查看元件列表，资源树和已使用的元件关系。"
              },
              {
                element:".sidebar_ele_list_",
                intro: "元件列表。"
              },
              {
                element: "div.sidebar_ele_list_ > div:nth-child(4)",
                intro: "拖拽或点击使用元件。"
              },
              {
                element: "#systeminfowindow",
                intro: "这里将显示系统信息。"
              },
              {
                element: 'body > div.geMenubarContainer > div.geMenubar',
                intro: "菜单栏。",
                position: 'bottom'
              },
              {
                element: "body > div.geToolbarContainer > div.geToolbar > a:nth-child(1)",
                intro: '使用向导新建工程。',
                position: 'right'
              },
              {
              ...
              {
                intro: 'Get it, use CloudPSS.'
              }
            ]
          });

	if (cookiesample.first ==undefined){
		addCookie('first',1,90);
		intro.start();
		return intro;
	}else {
		return;
...
EditorUi.prototype.addOCSIntro = function(){
	var intro = introJs();
	var oscg = insertOSCG2Graph(314);
	var outputchannel = insertOSCG2Graph(323,true);

	var osc_node = this.editor.graph.view.getState(oscg).shape.node;
	var ouyputchannel_node = this.editor.graph.view.getState(outputchannel).shape.node;
	this.editor.graph.scrollCellToVisible(oscg, 1);
	intro.setOptions({
	  	showStepNumbers: false,
	    steps: [
		  {
	        intro: "没有检测到测量元件！请按照步骤设置测量元件。"
	      },
	      {
	      	element:ouyputchannel_node,
	        intro: "将OutputChannel接入你想要测量的节点。"
	      },
	      {
	        element:osc_node,
	        intro: "在示波器分组中选中OutputChannel。"
	      }
	    ]
	  });
	intro.start();
...
EditorUi.prototype.libraryLoaded = function (file, images, optionalTitle) {
    ...
    var addImages = mxUtils.bind(this, function (imgs, content) {
        if (dropTarget == null) {
            ...
            mxUtils.write(dropTarget, '拖拽至此处');
        }
        content.appendChild(dropTarget);
    });

    // Adds new sidebar entry for this library
    var contentDiv = this.sidebar.addPalette('scratchpad', '便签', true, mxUtils.bind(this, function (content) {
