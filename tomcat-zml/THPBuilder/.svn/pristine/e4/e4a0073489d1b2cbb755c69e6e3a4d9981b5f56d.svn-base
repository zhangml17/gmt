...
Format.prototype.AfterRefresh = function() {
    ...
    /*for(var scel in scell){
    	console.log(scell[0],'这是当前选中的元素');
    	scell[0].geometry.height=0;
    }*/
...
/**
 * Adds the given color option.这里应该是改变颜色的方法
 */
...
BaseFormatPanel.prototype.addArrow = function(elt, height)
{
	...
	arrow.innerHTML = '<img border="0" src="' + ((mxClient.IS_SVG) ? 'data:image/png;base64,' :
		IMAGE_PATH + '/dropdown.png') + '" style="margin-top:-11px;">';
...
DiagramFormatPanel.prototype.addView = function(div)
{
	...
	if (graph.isEnabled())
	{
		...
		if (false)//if (this.showBackgroundImageOption)
		{
...
DiagramFormatPanel.prototype.addPaperSize = function(div)
{
	...
	var accessor = PageSetupDialog.addPageFormatPanel(div, 'formatpanel', graph.pageFormat, function(pageFormat)
	{
			...
			graph.pageFormat.height != pageFormat.height)
		{
            ...
            ui.myRuler.api.setSize(pageFormat.width-pageFormat.width%40+50, pageFormat.height-pageFormat.height%40+50);
		}
	});
...
ParameterPanel.prototype.addParamTagProfile = function(div) {
    ...
    //check text   这里sText截取的是cell.style的第一个分号前面的键值对
    ...
    if(sText == "text")
	{
        ...
        function UpdateTagValue() {
            ...
            graph.getModel().execute(new mxCellAttributeChange(cell,'',''));
        }   
    		...
    		pList.forEach(function (obj) {
                ...
                if(obj.type=="booleanRadio"){
                    ...
                    mxEvent.addListener(valueinput, 'click', function () {
                        ...
                        //处理如果是控制显示隐藏的开关操作后，
                        if(obj.label==='隐藏'){
                        	cell.showhide=(this.className).substr(-6,6);
                        }
                    });
                }
                //操作文字的显示隐藏
                if(obj.type=="isValShow"){
                    var valueinput = document.createElement('button');
                    valueinput.className = 'auto_set_btn'+(gObj[obj.name]?' btn_on':' btn_off');
                    valueinput.style.float = 'right';
                    valueinput.style.marginRight='56px';
                    valueinput.name = obj.name;
                    var tempDiv = containerdiv.cloneNode(false);
                    tempDiv.style.height = '30px';
                    tempDiv.appendChild(valuespan);
                    ...
                    div.appendChild(tempDiv);
                    mxEvent.addListener(valueinput, 'click', function () {
                        this.className = 'auto_set_btn'+(gObj[this.name]?' btn_on':' btn_off');
                        gObj.setAttribute(this.name,gObj[this.name]?false:true);
                    });
                }
                ...
                if(obj.type=="bgground"){
                    ...
                    var applyC = function (color) {
                    	...
                    	gObj[obj.name]=color;
                    	gObj.bgColor = color;
                    	gObj.setAttribute(gObj[this.name],color);
                    };
                ...
                if(obj.type=="foreground"){
                    ...
                    var applyC = function (color) {
                    	btnC.innerHTML = '<div style="width:36px;height:12px;margin:3px;border:1px solid black;background-color:'+(color != null ? color : '#f00') + ';"></div>';
                    	gObj[obj.name]=color;
//                    	gObj.element.children[0].style.backgroundColor = color;
                    	gObj.setAttribute(this[obj.name],color);
                    };
                ...
                //change direction
                if(obj.type==='directButton'){
                    var label1=document.createElement('label');
                    label1.style.float='right';
                    var radio1=document.createElement('input');
                    radio1.type='radio';
                    radio1.value='0';
                    radio1.name='direction';
                    if(!gObj.direction){
                        radio1.checked='checked';
					}
                    label1.appendChild(radio1);
                    label1.appendChild(document.createTextNode('水平'));
                    var label2=document.createElement('label');
                    ...
                    radio2.type='radio';
                    radio2.value='1';
                    radio2.name='direction';
                    if(gObj.direction){
                        radio2.checked='checked';
                    }
                    label2.appendChild(radio2);
                    label2.appendChild(document.createTextNode('竖直'));
                    var tempDiv = containerdiv.cloneNode(false);
                    tempDiv.style.height = '30px';
                    tempDiv.appendChild(valuespan);
                    tempDiv.appendChild(label2);
                    ...
                    mxEvent.addListener(radio1, 'click', function (evt) {
                        if(gObj.direction){
                            ...
                            cell.geometry.width=height;
                            cell.geometry.height=width;
                            gObj.setAttribute('direction',0);
						}
                    });
                    mxEvent.addListener(radio2, 'click', function (evt) {
                        if(!gObj.direction){
                            var width=cell.geometry.width;
                            var height=cell.geometry.height;
                            cell.geometry.width=height;
                            cell.geometry.height=width;
                            gObj.setAttribute('direction',1);
                        }
                    });
                };
                if(obj.type=='tileUrl'){
                    var valueinput = document.createElement('input');
                        ...
                        valueinput.style.width = '100px';
                        valueinput.style.float = 'right';
                    	valueinput.style.marginRight='26px';
                        valueinput.name = obj.name;
                    var tempDiv = containerdiv.cloneNode(false);
						...
						div.appendChild(tempDiv);
						mxEvent.addListener(valueinput, 'change', function () {
							gObj.setTileUrl(this.value);

						});
                };
                if(obj.type=='tileType'){
                    var label1=document.createElement('label');
                    var radio1=document.createElement('input');
                        radio1.type='radio';
                        radio1.value='0';
                        radio1.name='tileType';
                        radio1.checked='checked';
                        label1.appendChild(radio1);
                        label1.style.marginRight='22px';
                    	label1.style.float='right';
                        label1.appendChild(mxUtils.write(radio1,'标准'));
                    var label2=document.createElement('label1');
                        label2.style.marginRight = '26px';
                        label2.style.float='right';
                    var radio2=document.createElement('input');
                        radio2.type='radio';
                        radio2.value='1';
                        radio2.name='tileType';
                        label2.appendChild(radio2);
                        label2.appendChild(mxUtils.write(radio2,'国标'));
                    var tempDiv = containerdiv.cloneNode(false);
                        tempDiv.style.height = '30px';
                        tempDiv.style.width='200px';
                ...
                if(obj.type=='coordinate'){
                    var coorspan=valuespan.cloneNode(false); //Lat
                        coorspan.innerHTML='中心坐标：';     //centerLng 中心点经度 centerLat 中心点纬度
                    var Lnginput=document.createElement('input');
                        Lnginput.style.width='60px';
                        Lnginput.style.marginLeft='4px';
                    	Lnginput.style.float='right';
                        Lnginput.style.lineHeight='16px';
                    var latinput=Lnginput.cloneNode(false);
                    	latinput.style.float='right';

                    var coordiv=containerdiv.cloneNode(false);
                        coordiv.style.width='200px';
                        coordiv.style.height = '30px';
                        coordiv.appendChild(coorspan);
                        coordiv.appendChild(Lnginput);
                        coordiv.appendChild(latinput);
                        div.appendChild(coordiv);
						Lnginput.value=gObj.centerLng;
                    	latinput.value=gObj.centerLat;
                    mxEvent.addListener(Lnginput, 'change', function () {
                    	var lngval=parseFloat(Lnginput.value);
                        var point=[lngval, gObj.centerLat];
							gObj.setCenter(point);
                    });
                    mxEvent.addListener(latinput, 'change', function () {
                        var latval=parseFloat(latinput.value);
                        var point=[Lnginput.value,latval];
                        	gObj.setCenter(point);
                    });
                };
                if(obj.type=='zoom'){
                    var zoomspan=valuespan.cloneNode(false);
                        zoomspan.innerHTML='缩放级别：';
                        zoomspan.style.float='left';
                    if(!gObj[obj.name] && gObj[obj.name]!==0){
                        gObj[obj.name]='18';
                    }
                    var p=document.createElement('div');
                ...
                if(obj.type=="dataSrc"){
                  ...
                  //这里数据源是下拉框
                    ...
                    if(obj.label && obj.label==='测点表'){
                    	var gType=gObj.gobjType
                    	if(gType==='GChartTrend' || gType==='GChartPie' || gType==='GChartScatter'){
                    		var select2=document.createElement('select');
		                    	select2.style.float='right';
		                    	select2.style.width='68px';
		                    	select2.style.lineHeight='32px';
	                	...
	                	var meterselect=document.createElement('select');
		                	meterselect.style.float='right';
		                	meterselect.style.width='100px';
		                	meterselect.style.lineHeight='32px';
		                	for(var key in meter){
	                            var op=document.createElement('option');
	                            op.style.width='100%';
	                            op.style.textOverflow='ellipsis';
            ...
            mxEvent.addListener(btnLink, 'click', function () {
                if (cell != null){
                	if(cell.value.gobjType=='GChartPie'){
                        var dlg = new DataIdPieDialogEX(ui, cell, this.parentNode.name, this.previousSibling);
                        ui.showDialog(dlg.container, 1010, 642, true, false);
                        //$("#webContainer").attr("src", 'dbquery/#/selector');
                    }
                    if(cell.value.gobjType=='GChartTrend' || cell.value.gobjType=='GChartScatter'){
                        var dlg = new DataIdTrendDialogEX(ui, cell, this.parentNode.name, this.previousSibling);
                        ui.showDialog(dlg.container, 1010, 642, true, false);
                    }
                }
            });
