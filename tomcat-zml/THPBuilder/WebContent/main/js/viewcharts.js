 ...
 */
$(function () {
    objPoints = JSON.parse(localStorage.getItem('obj_point'));
    if(objPoints) {
        var html = '';
...
function  initchartData() {
    ...
    for(var i=0;i<checkeds.length;i++) {
        if(checkeds[i].getElementsByTagName("input")['checked'].checked) {
            var value=checkeds[i].getElementsByTagName("input")['checked'].value;
            pointIDs.push(parseInt(value));
            // if(pointIDs){
            //     pointIDs +=",";
            // }
    ...
    var startdate=pretime;
    var enddate=time;
    var tbname="hdr_analog";
    Viewchart.chartId="mychart1";
    Viewchart.getDatabyServer(tbname,pointIDs,startdate,enddate);
}
